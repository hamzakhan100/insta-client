...
const Home = () => {
  ...
  useEffect(() => {
    (async () => {
      try {
        const result = await postServices.allpost(config);
  ...
  const likePost = async (id) => {
    ...
    const result = await postServices.likePost(obj, obj2);
  ...
  const UnlikePost = async (id) => {
    ...
    const result = await postServices.unlike(obj, obj2);
  ...
  const makeComment = async (text, postId) => {
    ...
    const result = await postServices.comment(obj, obj2);
    const newData = data.map((item) => {
      if (result.data._id === item._id) return result.data;
      else return item;
    });
